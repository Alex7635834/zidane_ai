...
def predict(home_team, away_team, neutral=True):
    ...
    return [winner, proba, lambda_home, lambda_away]

def create_dataset_tournament(teams):
    retrieve_team_data()
    results = pd.read_csv("../data/results.csv")

    results = results[
        results["home_team"].isin(teams) | results["away_team"].isin(teams)
    ].copy()

    results.to_csv("../data/results_tournament.csv", index=True)
    return results

def predict_tournament(home_team, away_team, df, neutral=True):
